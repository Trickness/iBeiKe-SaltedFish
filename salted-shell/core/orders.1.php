...
function create_order_from_user($order_submitter,$order_type,$goods_id,$delivery_fee,$purchase_amount,$single_cost,$offer){
    ...
    $sql = "SELECT goods_title,goods_status,remain,goods_owner,$db_users_table.student_info FROM $db_goods_table INNER JOIN $db_users_table ON $db_goods_table.goods_owner = $db_users_table.student_id WHERE goods_id='$goods_id'";
    ...
    if($result){
        ...
        $user_info = json_decode(urldecode($info['student_info']),true);
    }else{
    ...
    if ($result){
        ...
        // 短信测试
        $phone_number = $user_info['phone_number']['value'];
        $sms = new OrderSms($phone_number,$sms_apikey,$sms_tpl_id);
        $sms_res = $sms->create_order(trim($order_submitter),trim('"'.$goods_title.'"'),trim($purchase_amount),trim($offer),'商城');
        // 短信测试
